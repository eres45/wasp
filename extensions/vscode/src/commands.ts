...
function getFullScreenTab() {
  ...
  return tabs.find((tab) =>
    (tab.input as any)?.viewType?.endsWith("waspcode.waspcodeGUIView"),
  );
}
...
function focusGUI() {
  ...
  if (fullScreenTab) {
    ...
    // focus sidebar
    vscode.commands.executeCommand("waspcode.waspcodeGUIView.focus");
...
const getCommandsMap: (
  ...
  return {
    "waspcode.acceptDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "waspcode.rejectDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "waspcode.defaultQuickAction": async (args: QuickEditShowParams) => {
      captureCommandTelemetry("defaultQuickAction");
      vscode.commands.executeCommand("waspcode.focusEdit", args);
    },
    "waspcode.customQuickActionSendToChat": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "waspcode.customQuickActionStreamInlineEdit": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "waspcode.codebaseForceReIndex": async () => {
      core.invoke("index/forceReIndex", undefined);
    },
    "waspcode.rebuildCodebaseIndex": async () => {
      core.invoke("index/forceReIndex", { shouldClearIndexes: true });
    },
    "waspcode.docsIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: false });
    },
    "waspcode.docsReIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: true });
    },
    "waspcode.focusContinueInput": async () => {
    ...
    "waspcode.focusContinueInputWithoutClear": async () => {
      const isContinueInputFocused = await sidebar.webviewProtocol.request(
        "isContinueInputFocused",
        undefined,
        false,
    ...
    "waspcode.writeDocstringForCode": async () => {
    ...
    "waspcode.optimizeCode": async () => {
      captureCommandTelemetry("optimizeCode");
      streamInlineEdit("optimize", "Optimize this code");
    },
    "waspcode.fixGrammar": async () => {
    ...
    "waspcode.clearConsole": async () => {
      consoleView.clearLog();
    },
    "waspcode.viewLogs": async () => {
      captureCommandTelemetry("viewLogs");
      vscode.commands.executeCommand("workbench.action.toggleDevTools");
    },
    "waspcode.debugTerminal": async () => {
    ...
    "waspcode.shareSession": async (sessionId: string | undefined) => {
      if (!sessionId) {
    ...
    "waspcode.viewHistory": () => {
      vscode.commands.executeCommand("waspcode.navigateTo", "/history", true);
    },
    "waspcode.focusContinueSessionId": async (
    ...
    "waspcode.applyCodeFromChat": () => {
      void sidebar.webviewProtocol.request("applyCodeFromChat", undefined);
    },
    "waspcode.openConfigPage": () => {
      vscode.commands.executeCommand("waspcode.navigateTo", "/config", false);
    },
    "waspcode.selectFilesAsContext": async (
      firstUri: vscode.Uri,
      uris: vscode.Uri[],
    ) => {
      if (uris === undefined) {
      ...
      for (const uri of uris) {
        ...
        if (isDirectory) {
          for await (const fileUri of walkDirAsync(uri.toString(), ide, {
            source: "vscode waspcode.selectFilesAsContext command",
          })) {
    ...
    "waspcode.forceAutocomplete": async () => {
    ...
    "waspcode.openTabAutocompleteConfigMenu": async () => {
      ...
      quickPick.onDidAccept(() => {
        ...
        if (targetStatus !== undefined) {
          ...
          vscode.commands.executeCommand("waspcode.focusContinueInput");
        } else if (selectedOption === "$(screen-full) Open full screen chat") {
          vscode.commands.executeCommand("waspcode.openInNewWindow");
        } else if (selectedOption === "$(gear) Open settings") {
          vscode.commands.executeCommand("waspcode.navigateTo", "/config");
        }
    ...
    "waspcode.navigateTo": (path: string, toggle: boolean) => {
      sidebar.webviewProtocol?.request("navigateTo", { path, toggle });
      focusGUI();
    },
    "waspcode.startLocalOllama": () => {
      startLocalOllama(ide);
    },
    "waspcode.startLocalLemonade": () => {
      startLocalLemonade(ide);
    },
    "waspcode.installModel": async (
      modelName: string,
    ...
    "waspcode.convertConfigJsonToConfigYaml": async () => {
        ...
        .then(async (selection) => {
          if (selection === "Read the docs") {
            await vscode.env.openExternal(
              vscode.Uri.parse("https://docs.waspcode.dev/yaml-migration"),
            );
          }
        });
    },
    "waspcode.enterEnterpriseLicenseKey": async () => {
    ...
    "waspcode.toggleNextEditEnabled": async () => {
    ...
    "waspcode.openInNewWindow": async () => {
      focusGUI();
      ...
      vscode.commands.executeCommand("waspcode.newSession");
      ...
      const sessionLoader = panel.onDidChangeViewState(() => {
        ...
        if (sessionId) {
          vscode.commands.executeCommand(
            "waspcode.focusContinueSessionId",
            sessionId,
          );
        }
        panel.reveal();
