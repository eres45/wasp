...
For personal use you can set the secret in the hub at https://waspcode.dev/settings/secrets or pass it to the CLI environment.
Org-level secrets can only be used for MCP by Background Agents (https://docs.waspcode.dev/hub/agents/overview) when \"Include in Env\" is enabled for the secret.`;
        if (this.isHeadless) {
